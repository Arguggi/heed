...
drawUI st = [ui]
  where
    ...
    ui =
        C.center $
        (txt "Feed Url " <+> errorMsg (st^.urlMessage)) <=>
        hLimit 100 e1 <=>
...
-- TODO
sendNewFeedData :: (MonadIO m) => WS.Connection -> T.Text -> Int -> m ()
sendNewFeedData conn url every = void . liftIO . forkIO $ WS.sendBinaryData conn (encode (NewFeed url every))

isValidUrl :: T.Text -> Maybe T.Text
isValidUrl url = case parseURI (T.unpack url) of
    Nothing -> Just "Invalid url"
    Just uri -> if validUriScheme (uriScheme uri) then Nothing else Just "Invalid scheme"
...
isValidUpdate update = case readEither (T.unpack update) of
    Left _ -> Just "Not a valid number"
    Right int -> if int < (0 :: Int) then Just "Must be positive" else Nothing
...
addVty state = do
    ...
    let finalStatus = if st ^. adding then "Adding " <> (headDef "" . E.getEditContents) (st^.urlEdit) else "Not adding new feed"
