...
prop_insertInOrder :: (Show e, Arbitrary e, Ord e) => e -> BL.List ListName e -> Bool
prop_insertInOrder e list = case list ^. BL.listSelectedL of
    Nothing -> new ^. BL.listSelectedL == Just 0
    (Just _)-> (snd <$> BL.listSelectedElement new) == (snd <$> BL.listSelectedElement list)
        && isSorted (Vec.toList (new ^. BL.listElementsL))
    where new = insertInOrder e list
...
instance (Arbitrary n, Arbitrary e, Ord e) => Arbitrary (BL.List n e) where
  arbitrary = do
    name <- arbitrary
    l <- orderedList
