...
import Network.Wai                      (Request, requestHeaders, Application)
import Network.Wai.Handler.Warp         (run)
import Servant.API                      ((:<|>) ((:<|>)), (:>), JSON, Post, ReqBody
                                        , FormUrlEncoded)
import Servant.API.Experimental.Auth    (AuthProtect)
import Servant                          (throwError, throwError, err303, ServantErr(..), NoContent)
import Servant.Server                   ( Context ((:.), EmptyContext),
                                         err401, errBody, Server,
...
import Network.WebSockets (ServerApp, defaultConnectionOptions, acceptRequest, sendBinaryData, receiveData, forkPingThread)
...
genAuthMain conf = run 8080 (serveWithContext genAuthAPI (genAuthServerContext conf) (genAuthServer conf))
...
type AuthGenAPI = "login" :> (ReqBody '[FormUrlEncoded] AuthData :> Post '[JSON] NoContent :<|> Raw)
    :<|> AuthProtect "cookie-auth" :> Raw
...
lookupTok conf cookies = case Map.lookup authCookieName $ Map.fromList (parseCookies cookies) of
